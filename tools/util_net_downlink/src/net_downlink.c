...
int main( int argc, char** argv )
{
    ...
    while( ( i = getopt( argc, argv, "b:c:f:hij:l:p:r:s:t:x:z:P:m:d:q:" ) ) != -1 )
    {
        ...
        case 'b': /* -b <uint>  LoRa modulation bandwidth */
            ...
            if( ( j != 1 ) || ( ( arg_u != 125 ) && ( arg_u != 250 ) && ( arg_u != 500 ) && ( arg_u != 200 ) &&
                                ( arg_u != 400 ) && ( arg_u != 800 ) ) )
            {
        ...
        case 'c': /* -c <string>  LoRa Coding Rate */
            ...
            if( ( j != 1 ) || ( ( strcmp( arg_s, "4/5" ) != 0 ) && ( strcmp( arg_s, "4/6" ) != 0 ) &&
                                ( strcmp( arg_s, "4/7" ) != 0 ) && ( strcmp( arg_s, "4/8" ) != 0 ) &&
                                ( strcmp( arg_s, "4/5LI" ) != 0 ) && ( strcmp( arg_s, "4/6LI" ) != 0 ) &&
                                ( strcmp( arg_s, "4/8LI" ) != 0 ) ) )
            {
    ...
    while( ( quit_sig != 1 ) && ( exit_sig != 1 ) )
    {
        ...
        x = getnameinfo( ( struct sockaddr* ) &dist_addr, addr_len, host_name, sizeof host_name, port_name,
                         sizeof port_name, NI_NUMERICHOST );
        if( x == -1 )
        {
        ...
        if( databuf_up[3] == PKT_PUSH_DATA )
        {
            if( log_fname != NULL )
            {
                ...
                {
                    fprintf( log_file,
                             "tmst,chan,rfch,freq,stat,modu,datr,bw,codr,rssi,"
                             "lsnr,size,data\n" );
                    is_first = false;
                }
    ...
    if( rxpk_array != NULL )
    {
        ...
        for( i = 0; i < rxpk_nb; i++ )
        {
            ...
            else if( strcmp( str, "FSK" ) == 0 )
            {
                ...
                fprintf( file, ",%.1f,", json_value_get_number( val ) ); /* lsnr field is left empty */
            }
            else
            {
            ...
            {
                printf(
                    "ERROR: mismatch between .size and .data size once converter to "
                    "binary\n" );
                json_value_free( root_val );
                return;
            }
...
static void usage( void )
{
    ...
    printf(
        " -j <uint>:<float>  Number of channels to jump + explicit offset in "
        "MHz between channels\n" );
    printf( " -b <uint>          LoRa bandwidth in kHz [125, 250, 500, 200, 400, 800]\n" );
    printf( " -s <uint>          LoRa Spreading Factor [5-12]\n" );
    printf(
        " -c <string>        LoRa Coding Rate [\"4/5\", \"4/6\", \"4/7\", \"4/8\", \"4/5LI\", \"4/6LI\", "
        "\"4/8LI\"]\n" );
...
static void* thread_down( const void* arg )
{
    ...
    while( !exit_sig && !quit_sig && ( pkt_sent < nb_loop ) && ( nb_loop > 0 ) )
    {
        ...
        /* Display info about the sender */
        x = getnameinfo( ( struct sockaddr* ) &dist_addr_down, addr_len_down, host_name, sizeof host_name, port_name,
                         sizeof port_name, NI_NUMERICHOST );
        if( x == -1 )
        {
            printf( "ERROR: getnameinfo returned %s \n", gai_strerror( x ) );
        ...
        else
        {
            ...
            byte_nb = sendto( params->sock, ( void* ) databuf_down, strlen( serialized_string ) + 4, 0,
            ...
            {
                printf(
                    "<-  pkt out, PULL_RESP for host %s (port %s), %i bytes sent "
                    "for downlink (%d)\n",
