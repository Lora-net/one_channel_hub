...
(______/|_____)_|_|_| \__)_____)\____)_| |_|
 ...
 Description:
    ...
    Network packet receiver, receives UDP packets from a running packet
forwarder.
...
static int exit_sig = 0; /* 1 -> application terminates cleanly (shut down
                            hardware, close open files, etc) */
static int quit_sig =
    0; /* 1 -> application terminates without shutting down the hardware */
...
/* Thread variables */
static pthread_mutex_t mx_sockaddr =
    PTHREAD_MUTEX_INITIALIZER; /* control access to the sockaddr info */
...
int main(int argc, char **argv) {
  ...
  /* Downlink variables */
  thread_params_t thread_params = {.nb_loop = 0,
                                   .delay_ms = 1000,
  ...
  while ((i = getopt(argc, argv, "b:c:f:hij:l:p:r:s:t:x:z:P:m:d:q:")) != -1) {
    switch (i) {
    case 'h':
      usage();
      return EXIT_SUCCESS;
    ...
    case 'c': /* -c <string>  LoRa Coding Rate */
      ...
      if (j != 1) {
        ...
        strncpy(thread_params.coding_rate, arg_s,
                sizeof thread_params.coding_rate);
        thread_params.coding_rate[sizeof thread_params.coding_rate - 1] =
    ...
    case 'm': /* -m <string> Modulation */
      ...
      if (j != 1) {
        ...
        strncpy(thread_params.modulation, arg_s,
                sizeof thread_params.modulation);
        thread_params.modulation[sizeof thread_params.modulation - 1] =
            '\0'; /* ensure string termination */
      }
      break;
    ...
    case 'd': /* <uint> FSK frequency deviation */
      j = sscanf(optarg, "%u", &arg_u);
      if ((j != 1) || (arg_u < 1) || (arg_u > 250)) {
        printf("ERROR: argument parsing of -d argument\n");
        usage();
        return EXIT_FAILURE;
      } else {
    ...
    case 'q': /* <float> FSK bitrate */
      j = sscanf(optarg, "%lf", &arg_f);
      if ((j != 1) || (arg_f < 0.5) || (arg_f > 250)) {
        printf("ERROR: argument parsing of -q argument\n");
        usage();
        return EXIT_FAILURE;
      } else {
  ...
  /* Start message */
  printf("+++ Start of LoRa network server utility +++\n");
  ...
  if (log_fname) {
    log_file = fopen(
        log_fname, "w+"); /* create log file, overwrite if file already exist */
    if (log_file == NULL) {
  ...
  while ((quit_sig != 1) && (exit_sig != 1)) {
    ...
    byte_nb = recvfrom(sock, databuf_up, sizeof databuf_up, 0,
    ...
    x = getnameinfo((struct sockaddr *)&dist_addr, addr_len, host_name,
    ...
    printf(" -> pkt in , host %s (port %s), %i bytes", host_name, port_name,
           byte_nb);
    ...
    /* Don't touch the token in position 1-2, it will be sent back "as is" for
     * acknowledgement */
    ...
    case PKT_PUSH_DATA:
      printf(", PUSH_DATA from gateway 0x%08X%08X\n", (uint32_t)(gw_mac >> 32),
             (uint32_t)(gw_mac & 0xFFFFFFFF));
      ack_command = PKT_PUSH_ACK;
      no_ack = false;
      printf("<-  pkt out, PUSH_ACK for host %s (port %s)", host_name,
             port_name);
      break;
    ...
    case PKT_PULL_DATA:
      ...
      no_ack = false;
      printf("<-  pkt out, PULL_ACK for host %s (port %s)", host_name,
             port_name);
      /* Record who sent the PULL_DATA for the downlink thread to known where to
       * send PULL_RESP */
    ...
    if (no_ack == false) {
      ...
      byte_nb = sendto(sock, (void *)databuf_ack, 4, 0,
                       (struct sockaddr *)&dist_addr, addr_len);
      if (byte_nb == -1) {
        printf(", send error:%s\n", strerror(errno));
      } else {
    ...
    if (databuf_up[3] == PKT_PUSH_DATA) {
      if (log_fname != NULL) {
        if (is_first == true) {
          fprintf(log_file, "tmst,chan,rfch,freq,stat,modu,datr,bw,codr,rssi,"
                            "lsnr,size,data\n");
          is_first = false;
        }
  ...
  printf("INFO: Exiting LoRa network server utility\n");
  ...
  if (rxpk_array != NULL) {
    ...
    for (i = 0; i < rxpk_nb; i++) {
      ...
      if (json_value_get_type(val) != JSONString) {
        printf("ERROR: wrong type for modu\n");
        json_value_free(root_val);
        return;
      }
      ...
      if (strcmp(str, "LORA") == 0) {
        ...
        fprintf(file, ",%d,,",
                (uint32_t)json_value_get_number(
                    val)); /* bw,codr fields are left empty */
        ...
        fprintf(file, ",%.1f,",
                json_value_get_number(val)); /* lsnr field is left empty */
      } else {
      ...
      if (x != size) {
        printf("ERROR: mismatch between .size and .data size once converter to "
               "binary\n");
        json_value_free(root_val);
        return;
      }
...
static void usage(void) {
  ...
  printf(" -j <uint>:<float>  Number of channels to jump + explicit offset in "
  ...
  printf(" Send downlinks:\n");
  printf("   ./net_downlink -f 865.1 -s 7 -b 125 -r 8 -t 500 -x 10 -P 1730\n");
  printf(" Trigger continuous TX:\n");
  printf("   ./net_downlink -f 865.1 -s 11 -x 1 -r 65535 -P 1730\n");
}
...
void prepare_downlink_json(const thread_params_t *params, uint8_t rf_chain,
                           uint32_t pkt_sent, JSON_Value *root_val) {
  int j;
  ...
  if (root_obj == NULL) {
    ...
    freq =
        params->freq_mhz + ((pkt_sent % params->freq_nb) * params->freq_step);
    ...
    for (j = 0; j < params->pl_size; j++) {
      payload[params->pl_size - (j + 1)] =
          (uint8_t)((pkt_sent >> (j * 8)) & 0xFF);
    }
    /* Convert payload to base64 */
    j = bin_to_b64(payload, params->pl_size, (char *)(payload_b64),
...
static void *thread_down(const void *arg) {
  ...
  while (!exit_sig && !quit_sig && (pkt_sent < nb_loop) && (nb_loop > 0)) {
    ...
    /* Display info about the sender */
    x = getnameinfo((struct sockaddr *)&dist_addr_down, addr_len_down,
                    host_name, sizeof host_name, port_name, sizeof port_name,
                    NI_NUMERICHOST);
    if (x == -1) {
      printf("ERROR: getnameinfo returned %s \n", gai_strerror(x));
      usleep(10000); /* 10 ms */
    ...
    if (root_val == NULL) {
      ...
      memcpy(&databuf_down[4], (uint8_t *)serialized_string,
             strlen(serialized_string));
      byte_nb = sendto(params->sock, (void *)databuf_down,
                       strlen(serialized_string) + 4, 0,
                       (struct sockaddr *)&dist_addr_down, addr_len_down);
      if (byte_nb == -1) {
        printf("ERROR: failed to send downlink to socket - %s\n",
               strerror(errno));
      } else {
        printf("<-  pkt out, PULL_RESP for host %s (port %s), %i bytes sent "
               "for downlink (%d)\n",
